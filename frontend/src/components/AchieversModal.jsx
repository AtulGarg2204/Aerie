...
const AchieversModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/contact/achievers`,
        formData
      );
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-gray-500 bg-opacity-75 flex items-center justify-center p-4">
      <div className="relative bg-white rounded-lg overflow-hidden shadow-xl transform transition-all w-full max-w-md mx-auto">
        ...
        <div className="px-4 py-5 sm:p-6">
          <h3 className="text-xl font-semibold text-gray-900 mb-5 text-left">Book a Free Expert Counselling Session</h3>
          ...
          {submitStatus ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-left">
              <div>
                ...
                {validationMsg.name && (
                  <p className="mt-1 text-sm text-red-600 text-left" id="name-error">
                    {validationMsg.name}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {validationMsg.email && (
                  <p className="mt-1 text-sm text-red-600 text-left" id="email-error">
                    {validationMsg.email}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {validationMsg.phone && (
                  <p className="mt-1 text-sm text-red-600 text-left" id="phone-error">
                    {validationMsg.phone}
                  </p>
                )}
              </div>
